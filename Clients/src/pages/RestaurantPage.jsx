...
import { IconUser, IconMail, IconLock, IconMapPin, IconUtensils } from '../components/AuthIcons';
...
const iconClass = 'absolute left-3.5 top-1/2 -translate-y-1/2 text-primary/60 pointer-events-none';
const inputClass = 'w-full border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-sm outline-none bg-white focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all duration-200 placeholder:text-gray-400';
...
function RestaurantPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', location: '', cuisineType: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        localStorage.setItem('name', res.data.restaurant.name);
        showToast(`Welcome back, ${res.data.restaurant.name}!`, 'success');
        navigate('/restaurant/dashboard');
      } else {
        const res = await axios.post('http://localhost:5000/api/restaurant/register', form);
          ...
          <div className="md:col-span-7 p-6 md:p-8 bg-white">
            ...
            <div className="mb-5">
              ...
              <p className="text-ink-soft text-xs mt-1">
                {isLogin ? 'Welcome back! Sign in to your restaurant account.' : 'Join FoodBridge and start making a difference today.'}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              {!isLogin && (
                <div className={inputWrapClass}>
                  <IconUser size={18} className={iconClass} width={18} height={18} />
                  <input name="name" placeholder="Restaurant Name" value={form.name} onChange={handleChange} required className={inputClass} />
                </div>
              )}
              ...
              {isLogin && (
                <Link to="/restaurant/forgot-password" className="text-primary text-xs text-right -mt-1 hover:underline font-medium">
                  Forgot Password?
                </Link>
              )}
              ...
              {!isLogin && (
                <>
                  ...
                  <div className={inputWrapClass}>
                    <IconUtensils width={18} height={18} className={iconClass} />
                    <input name="cuisineType" placeholder="Cuisine Type (e.g. North Indian)" value={form.cuisineType} onChange={handleChange} required className={inputClass} />
                  </div>
                </>
              )}
            ...
            <div className="mt-6 pt-5 border-t border-gray-100 text-center">
              <button onClick={() => setIsLogin(!isLogin)} className="text-sm text-ink-soft hover:text-primary transition-colors duration-150">
                {isLogin
                  ? <span>Don't have an account? <span className="text-primary font-semibold">Register</span></span>
                  : <span>Already registered? <span className="text-primary font-semibold">Sign In</span></span>
                }
              </button>
            </div>
          </div>
